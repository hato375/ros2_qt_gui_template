...
STATUS_TOPIC = "/sample_processor/status"
        ...
        PythonLaunchDescriptionSource(
            PathJoinSubstitution(
                [
                    ...
                    "launch",
                    "component_status_demo.launch.py",
                ]
            )
        )
    )
...
class TestComponentStatusDemo(unittest.TestCase):
    @classmethod
    def setUpClass(cls):
        rclpy.init()
        cls.node = rclpy.create_node("component_status_demo_launch_test")
    ...
    def test_sample_status_matches_gui_yaml(self):
        deadline = time.monotonic() + 10.0
        expected_states = [
            ComponentStatus.STATE_READY,
            ComponentStatus.STATE_RUNNING,
        ]
        while time.monotonic() < deadline and (
            self.latest_status is None
            or self.latest_status.state not in expected_states
        ):
            rclpy.spin_once(self.node, timeout_sec=0.1)
        self.assertIsNotNone(self.latest_status)
        self.assertEqual(self.latest_status.component_id, "sample-processor-1")
        self.assertIn(self.latest_status.state, expected_states)

        self.assertTrue(self.parameter_client.wait_for_service(timeout_sec=10.0))
        names = [
            "component_monitor_names",
            "component_monitors.sample_processor.status_topic",
            "component_monitors.sample_processor.expected_component_id",
        ...
        self.assertEqual(values["component_monitor_names"], ["sample_processor"])
        self.assertEqual(
            values["component_monitors.sample_processor.status_topic"],
            "sample_processor/status",
        )
        self.assertEqual(
            values["component_monitors.sample_processor.expected_component_id"],
...
class TestComponentStatusDemoShutdown(unittest.TestCase):
